...
namespace AryoKesuma\LaravelInstaller\Middleware;
    ...
    public function handle($request, Closure $next)
    {
        if ($this->alreadyInstalled()) {
            ...
            switch ($installedRedirect) {
                ...
                case 'url':
                    $urlName = config('installer.installed.redirectOptions.url.name');
                    $data = config('installer.installed.redirectOptions.url.message');

                    return redirect($urlName)->with(['data' => $data]);
                    break;
